...
class MATHransformer:
    ...
    def handle_by_month(self):
        ...
        for month in (good_data if self.use_good_data else candidate):
            ...
            #tem['from_id'] = tem['from_id'].map(lambda x: "U" + str(x))  # user
            tem['from_id'] = tem['from_id'].map(lambda x: int(x))  # user
            #tem['to_id'] = tem['to_id'].map(lambda x: "U" + str(x))  # user
            tem['to_id'] = tem['to_id'].map(lambda x:int(x))  # user
            # 统计权重
            tem = tem.groupby(['from_id', 'to_id']).size().reset_index().rename(columns={0: 'weight'})
    ...
    def get_full_node_set(self):
        ...
        pd_data['node'] = pd_data['node'].map(lambda x:int(x))
        pd_data = (pd_data).sort_values(by='node',axis=0, ascending=True)
        #pd_data = pd_data.sort_index(axis=0, ascending=True)#排序
...
def transform(trans_type=None, use_good_data=False):
    ...
    math = MATHransformer(input_file="E:\\PycharmProjects\\hello\\data\\math\\0.input\\sx-mathoverflow.txt", output_base_path="E:\\PycharmProjects\\hello\\data\\math")
